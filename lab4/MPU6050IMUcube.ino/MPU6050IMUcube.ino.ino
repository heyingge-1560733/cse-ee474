...
void SetVars(void)
{
  ...
  //MAstar[0] = MAstar[0] + pitch - MAstar[0] / 5;
  //MAstar[1] = MAstar[1] + yaw - MAstar[1] / 5;
  //MAstar[2] = MAstar[2] + roll - MAstar[2] / 5;
    
  //Xan2 = -1 * MAstar[0] / 5.0 / fact; // convert degrees to radians.
  //Yan2 = -1 * MAstar[2] / 5.0 / fact;
  Xan2 = -1  * roll / fact;
  Yan2 = pitch / fact;
  Zan2 = -1  * yaw /fact;
...
void setup() {
  ...
  tft.setTextSize(2); // Set text size to normal, 2 is twice normal etc.
  tft.setTextColor(ILI9341_BLACK); // Set pixel color; 1 on the monochrome screen
...
void loop() {
  ...
  tft.setCursor(20,  45); tft.print((int16_t)pitch); 
  tft.setCursor(100, 45); tft.print((int16_t)yaw); 
  tft.setCursor(180, 45); tft.print((int16_t)roll);
